...
from detect_arrow import arrow_detect
import numpy as np
import os, glob, time
...
def callback(data):
  ...
  found, theta, orient, direction, output = arrow_detect(current_frame)
  if direction == 1:
      direction = 'Right'
  else:
      direction = 'Left'
  if found:
      print("Arrows detected:")
      output = cv2.putText(output, direction , (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
