...
class client():
      def __init__(self):
            ...
            self.info_sub = rospy.Subscriber('/mrt/camera/color/camera_info', CameraInfo, self.info_callback)
      ...
      def pixel_to_3d(self, x,y):
          ...
          # print(depth)
          ray = self.model.projectPixelTo3dRay((x,y))  # get 3d ray of unit length through desired pixel
...
def diff(q1,q2):#accepts tuples
      ...
      return abs(transformations.euler_from_quaternion(diff_q)[2]) # return yaw between two angles (quaternions)
