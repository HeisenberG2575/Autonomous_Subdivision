...
class client:
      ...
      def is_complete(self,pos_x, pos_y, q):
            for ps in self.completed_list:
                  if np.sqrt((ps[0] - pos_x)**2 + (ps[1] - pos_y)**2) < 0.7 and diff(q,ps[2])<5:#Replace 5 by 0.2 after adding orientation specific things
                        return True
            return False
      ...
      def arrow_detect(self):
            ...
            #cv2.waitKey(0)
            for cnt in contours:
                  if cv2.contourArea(cnt) < 150:#Increase to make it robust to false positive
                        continue
...
def just_ahead(pos_x, pos_y, q, off_dist = 0.5):
      ...
      offset = transformations.quaternion_multiply((q.x,q.y,q.z,q.w), (off_dist,0,0,0))
